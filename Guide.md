...
![image](https://hackmd.io/_uploads/HJweclY0Zl.png)
...
quadrantChart
    ...
    x-axis "偏技術" --> "偏創意"
    y-axis "偏工具" --> "偏平台"
    quadrant-1 "象限三：AI 創意生成"
    quadrant-2 "象限一：AI 編程工具"
    quadrant-3 "象限二：AI 工作流自動化"
    quadrant-4 "象限四：AI 對話 & Agent"
    "Claude Code": [0.2, 0.75]
    "Cursor": [0.3, 0.7]
    "GitHub Copilot": [0.25, 0.6]
    "n8n": [0.35, 0.3]
    "Zapier": [0.4, 0.25]
    "ComfyUI": [0.75, 0.7]
    "Midjourney": [0.8, 0.5]
    "Runway": [0.85, 0.55]
    "Claude Desktop": [0.5, 0.35]
    "ChatGPT": [0.55, 0.3]
    "Hermes": [0.45, 0.4]
```
...
LLM 的 context window 是有限的。如果你把一千個工具的詳細說明全塞進去，要麼塞不下、要麼塞下了但 LLM 注意力被稀釋。
...
### 5.2 Prompt 方法：RCAF 結構

好的 prompt 不是「唸咒語」，而是有結構的描述。記住這四個字母：
...
LLM 有個特性叫「Chain of Thought」——你叫它「先解釋再回答」，它的答案品質會明顯提升。因為解釋的過程強迫它把推理寫出來，自己也會發現「我這個假設好像不對」。

直接叫它「修」，它會在沒推理的狀態下亂改。先叫它「解釋」，它會在推理的狀態下精確改。

---

## 6. 現場 Demo 流程（建議跟著做一遍）
...
flowchart TD
    ...
    S3 --> S4["4. 出 Bug?<br/>用 5.5 Debug 流程"]
